...
class BayesianModel:
    ...
    def compile(self, priors: dict, POPho: bool = False, POLi: bool = False):
        ...
        with pm.Model() as model:
            ...
            mass = pm.Uniform(
                r"Mass [$M_{\odot}$]", lower=m_lower, upper=m_upper, shape=self.nStars
            )
            ...
            if "Age [Myr]" in priors.keys():
                if priors["Age [Myr]"]["dist"] == "normal":
                    age = pm.Normal(
                        "Age [Myr]",
                        mu=priors["Age [Myr]"]["mu"],
                        sigma=priors["Age [Myr]"]["sigma"],
                    )
                elif priors["Age [Myr]"]["dist"] == "uniform":
                    age = pm.Uniform(
                        "Age [Myr]",
                        upper=priors["Age [Myr]"]["upper"],
                        lower=priors["Age [Myr]"]["lower"],
                    )
                else:
            ...
            if "Distance [pc]" in priors.keys():
                if priors["Distance [pc]"]["dist"] == "normal":
                    distance = pm.Normal(
                        "Distance [pc]",
                        mu=priors["Distance [pc]"]["mu"],
                        sigma=priors["Distance [pc]"]["sigma"],
                        shape=self.nStars,
                    )
                elif priors["Distance [pc]"]["dist"] == "uniform":
                    distance = pm.Uniform(
                        "Distance [pc]",
                        upper=priors["Distance [pc]"]["upper"],
                        lower=priors["Distance [pc]"]["lower"],
                        shape=self.nStars,
                    )
                else:
            ...
            if self.Li_data is not None:
                Li_true = pm.Deterministic(
                    "Li*", np.log10(Li[:, 0]) + 3.3
                )  # [L_0] = 3.3
        ...
        if kind == "age":
            ...
            for x in mu:
                ...
                if z_score < threshold:
                    if l:
                        sns.kdeplot(
                            self.idata.posterior["Age [Myr]"][c],
                            ax=ax,
                            fill=True,
                            color="blue",
                        )
